-- Группировка рейтинга ТВ: общий рейтинг, смены, параллели и отдельные классы.
-- Выполните файл целиком в Supabase SQL Editor.

alter table public.screen_leaderboard_settings
  add column if not exists group_mode text not null default 'all';
alter table public.screen_leaderboard_settings
  add column if not exists selected_groups jsonb not null default '[]'::jsonb;
alter table public.screen_leaderboard_settings
  add column if not exists class_shift_map jsonb not null default '{}'::jsonb;

alter table public.screen_leaderboard_settings
  drop constraint if exists screen_leaderboard_settings_group_mode_check;
alter table public.screen_leaderboard_settings
  add constraint screen_leaderboard_settings_group_mode_check
  check (group_mode in ('all', 'shift', 'grade', 'class'));
...
as $function$
  with screen_owner as (
    ...
    group by bounds.group_mode, bounds.selected_groups, bounds.class_shift_map,
